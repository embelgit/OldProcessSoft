	...
	return list;
	 ...
	 /*To register New ItemName In Stock Table*/
		...
		public List getInwardProcess3List(HttpServletRequest request){
			...
			HibernateUtility hbu=null;
			Session session=null;
			List<Stock> supList=null;
		try{	

			hbu = HibernateUtility.getInstance();
			session = hbu.getHibernateSession();

			Query query=session.createSQLQuery("SELECT PKSTOCKID, PRODUCTNAME, QUANTITYINKG from stock order by PKSTOCKID ");
			//Query query = session.createQuery("from PurchaseBill2");
			List<Object[]> list = query.list();

			supList= new ArrayList<Stock>(0);
		...
		for (Object[] object : list) {
			...
			reports.setQuantityinkg(Double.parseDouble(object[2].toString()));
