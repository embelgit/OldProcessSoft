...
public class InwardProcess3Dao {
	
	public List getInwardProcess3Operaion() {
		...
		try
		{
			...
			Query query = session.createSQLQuery("SELECT pk_operation_details_id, operation_name FROM operation_details WHERE pk_operation_details_id=3");
	...
	public List getAllPurchaseOrderNo() {
		...
		try
		{
			hbu = HibernateUtility.getInstance();
			session = hbu.getHibernateSession();
			Query query = session.createSQLQuery("SELECT inward_processing_info_pk, purchase_order_no FROM inward_processing_info WHERE fk_operation_details_id = 3 AND purchase_order_no > 0 ORDER BY inward_processing_info_pk");
	...
	public List getChallanNumberForProcess3(String InwardProcessingInfoPk, String purchaseOrderNo) {
	...
	public List getAllChallanNumber() {
		...
		try
		{
			hbu = HibernateUtility.getInstance();
			session = hbu.getHibernateSession();
			Query query = session.createSQLQuery("SELECT inward_processing_info_pk, challan_no FROM inward_processing_info WHERE fk_operation_details_id = 3 AND purchase_order_no = 0  ORDER BY inward_processing_info_pk");
			//Query query = session.createSQLQuery("SELECT distinct pono FROM inwand_details");
	...
	public void saveInwardProcess3Details(InwardProcess3Hibernate sdb) {
	...
	public List getInwardProcess3List(HttpServletRequest request){
		...
		HibernateUtility hbu=null;
		Session session=null;
		List<InwardProcess3Bean> supList=null;
	try{	

		hbu = HibernateUtility.getInstance();
		session = hbu.getHibernateSession();

		Query query=session.createSQLQuery("SELECT operation_name, purchase_order_no, challan_no, send_to_operation_name, comment from inward_process3 where shop_id='"+shopid+"' ");
		//Query query = session.createQuery("from PurchaseBill2");
		List<Object[]> list = query.list();

		supList= new ArrayList<InwardProcess3Bean>(0);

	for (Object[] object : list) {
		
		InwardProcess3Bean reports = new InwardProcess3Bean();
