...
public class ExpenditurePaymentDao {
	...
	//get latest Transaction number in Expanduture_payment
		public List getExpenditurePaymentTxid() {
			HibernateUtility hbu=null;
			Session session=null;
			...
			try
			{
				hbu = HibernateUtility.getInstance();
				session = hbu.getHibernateSession();
				Query query = session.createSQLQuery("SELECT transaction_id , accountant_name FROM expenditure_payment ORDER BY transaction_id DESC LIMIT 1");
