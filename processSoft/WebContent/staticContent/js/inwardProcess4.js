...
function getChallanNo() {
	...
	if(purchaseOrderNo != "" || purchaseOrderNo !=null || purchaseOrderNo !="undefined") {
		...
		params["methodName"] = "getChallanNoForProcess4";
...
function validationInwardProcess4Details() {
...
//	var comment = $('#comment').val();
	...
	inwardProcess4Details();
	
}

function inwardProcess4Details() {

	document.getElementById('saveInwardProcess4DetailsBtn').disabled = true;
	...
	params["methodName"] = "saveInwardProcess4Details";
	

	$.post('/processSoft/jsp/utility/controller.jsp', params, function(data) {
		alert(data);
		location.reload();
		document.getElementById('saveInwardProcess4DetailsBtn').disabled = false;
	}).error(function(jqXHR, textStatus, errorThrown) {
		if (textStatus === "timeout") {
			$(loaderObj).hide();
