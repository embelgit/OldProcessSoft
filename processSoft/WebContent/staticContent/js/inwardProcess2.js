...
function getChallanNo() {
	...
	if(purchaseOrderNo != "" || purchaseOrderNo !=null || purchaseOrderNo !="undefined") {
		...
		params["methodName"] = "getChallanNoForProcess2";
...
function validationInwardProcess2Details() {
...
//	var comment = $('#comment').val();
	...
	inwardProcess2Details();
	
}

function inwardProcess2Details() {

	document.getElementById('saveInwardProcess2DetailsBtn').disabled = true;
	...
	params["methodName"] = "saveInwardProcess2Details";
	

	$.post('/processSoft/jsp/utility/controller.jsp', params, function(data) {
		alert(data);
		location.reload();
		document.getElementById('saveInwardProcess2DetailsBtn').disabled = false;
	}).error(function(jqXHR, textStatus, errorThrown) {
		if (textStatus === "timeout") {
			$(loaderObj).hide();
