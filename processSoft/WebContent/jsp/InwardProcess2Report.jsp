<!-- InwardProcess2List.jsp -->
<%@page import="com.Fertilizer.bean.InwardProcess2Bean"%>
<%@page import="com.Fertilizer.dao.InwardProcess2Dao"%>
...
<html>
	<head>
		...
		<title>Inward Process 2 Report</title>
		...
		%>
			  		<h2 class="form-name style_heading"><%if(abc.equals("marathi")){%><%=PropertiesHelper.marathiProperties.getProperty("godownDetails") %> <%}%> <%if(abc.equals("english")){%> Inward Process 2 <%=operationName%> Report <%}%> </h2>
			  	</div>		 
    </div>
  		<script type="text/javascript">
  			function Back()
  			{
  				window.location = "InwardProcess2.jsp" ;
  			}
...
<body id="dt_example" style="min-height:300px;">
	...
	List list12=dao.getInwardProcess2List(request);
	%>
			...
			<tbody>
   				<%
					for(int i=0;i<list12.size();i++){
						InwardProcess2Bean sr=(InwardProcess2Bean)list12.get(i);
				%>		
				<tr>
