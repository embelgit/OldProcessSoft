<!-- InwardProcess1List.jsp -->
<%@page import="com.Fertilizer.bean.GetSupplierDetails"%>
<%@page import="com.Fertilizer.dao.SupplierDetailsDao"%>
<%@page import="com.Fertilizer.bean.InwardProcess1Bean"%>
...
<html>
	<head>
		...
		%>
			  		<h2 class="form-name style_heading"><%if(abc.equals("marathi")){%><%=PropertiesHelper.marathiProperties.getProperty("godownDetails") %> <%}%> <%if(abc.equals("english")){%> Inward Process 1 <%=operationName%> List <%}%> </h2>
			  	</div>		 
    </div>
  		<script type="text/javascript">
  			function Back()
  			{
  				window.location = "InwardProcess1.jsp" ;
  			}
			...
			<tbody>
   				<%
					for(int i=0;i<list12.size();i++){
						InwardProcess1Bean sr=(InwardProcess1Bean)list12.get(i);
				%>		
				<tr>
